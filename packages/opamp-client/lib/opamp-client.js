...
// @ts-expect-error -- allow requiring package.json outside of `rootDir`
...
/**
 * A structural subset of `RemoteConfigStatus` accepted by
 * `setRemoteConfigStatus()`, so callers can pass a plain object.
 *
 * @typedef {Object} RemoteConfigStatusInput
 * @property {Uint8Array} lastRemoteConfigHash
 * @property {number} [status] - A `RemoteConfigStatuses` value.
 * @property {string} [errorMessage]
 */

/**
 * A structural subset of `EffectiveConfig` accepted by
 * `setEffectiveConfig()`, so callers can pass a plain object.
 *
 * @typedef {Object} AgentConfigFileInput
 * @property {Uint8Array} body
 * @property {string} contentType
 *
 * @typedef {Object} EffectiveConfigInput
 * @property {{configMap: Record<string, AgentConfigFileInput>}} [configMap]
 */
...
class OpAMPClient {
  ...
  constructor(opts) {
    ...
    this._instanceUid = normalizeInstanceUid(opts.instanceUid ?? genUuidv7());
    this._instanceUidStr = uuidStringify(this._instanceUid);
    this._capabilities = normalizeCapabilities(opts.capabilities);
    try {
      this._heartbeatIntervalMs =
        normalizeHeartbeatIntervalSeconds(opts.heartbeatIntervalSeconds) * 1000;
    } catch (err) {
      this._log.warn(
  ...
  setAgentDescription(desc) {
    const agentDescription = create(AgentDescriptionSchema, {
      ...
      nonIdentifyingAttributes: keyValuesFromObj(desc.nonIdentifyingAttributes),
    });
  ...
  setHeartbeatIntervalSeconds(n) {
    try {
      this._heartbeatIntervalMs = normalizeHeartbeatIntervalSeconds(n) * 1000;
    } catch (err) {
      this._log.warn(
   ...
   * handler.
   *
   * @param {RemoteConfigStatusInput} remoteConfigStatus - The
  ...
  setRemoteConfigStatus(remoteConfigStatus) {
    ...
    if (
      ...
      remoteConfigStatus.errorMessage !== this._remoteConfigStatus.errorMessage
    ) {
      isChanged = true;
      this._remoteConfigStatus.errorMessage = remoteConfigStatus.errorMessage;
    }
  ...
  /**
   * setEffectiveConfig sets the current EffectiveConfig and, if changed,
   * schedules sending a message to the server.
   *
   * May be called anytime after creation, including from the `onMessage`
   * handler. Requires the `ReportsEffectiveConfig` capability.
   *
   * @param {EffectiveConfigInput} effectiveConfig - For example:
   *      {
   *          configMap: {
   *              configMap: {
   *                  '': {body: <Uint8Array>, contentType: 'application/json'}
   *              }
   *          }
   *      }
   */
  setEffectiveConfig(effectiveConfig) {
    if (!this._hasCapReportsEffectiveConfig()) {
      throw new Error(
        'do not call `setEffectiveConfig()` when `ReportsEffectiveConfig` capability is not set'
      );
    }

    const ec = create(EffectiveConfigSchema, effectiveConfig);
    const ecSer = toBinary(EffectiveConfigSchema, ec);
    const isChanged =
      !this._effectiveConfig ||
      !isEqualUint8Array(this._effectiveConfigSer, ecSer);
    if (isChanged) {
      this._effectiveConfig = ec;
      this._effectiveConfigSer = ecSer;
      this._queue.push('EffectiveConfig');
      this._scheduleSendSoon();
    }
  }
  ...
  _hasCapReportsEffectiveConfig() {
    return (
      this._capabilities &
      BigInt(AgentCapabilities.AgentCapabilities_ReportsEffectiveConfig)
    );
  }
  ...
  _scheduleSendAfterErr() {
    ...
    const delayS = Math.min(300, Math.min(this._numSendFailures, 10) ** 3 + 29);
    ...
    for (let entry of queue) {
      switch (entry) {
        case 'ReportFullState':
          ...
          a2s.agentDescription = structuredClone(this._agentDescription);
          if (this._hasCapReportsRemoteConfig()) {
            a2s.remoteConfigStatus = structuredClone(this._remoteConfigStatus);
          }
          if (this._hasCapReportsEffectiveConfig() && this._effectiveConfig) {
            a2s.effectiveConfig = structuredClone(this._effectiveConfig);
          }
          break;
        ...
        case ServerErrorResponseType.ServerErrorResponseType_Unavailable:
          ...
          if (s2a.errorResponse.Details.case === 'retryInfo') {
            retryAfterMs = msFromRetryAfterNs(
              s2a.errorResponse.Details.value.retryAfterNanoseconds
            );
          }
          break;
  ...
  _processServerToAgent(s2a) {
    ...
    if (s2a.capabilities !== this._serverCapabilities) {
      ...
      if (
        ...
        !(
          ...
          BigInt(ServerCapabilities.ServerCapabilities_OffersRemoteConfig)
        )
      ) {
        this._log.debug(
    ...
    if (
      s2a.flags & BigInt(ServerToAgentFlags.ServerToAgentFlags_ReportFullState)
    ) {
      this._queue.push('ReportFullState');
    ...
    if (this._onMessage && Object.keys(onMessageData).length > 0) {
      ...
      queueMicrotask(() => {
        try {
          ...
          this._log.warn(err, 'ignoring exception from `onMessage` callback');
        }
      });
    }
  }
